...
import { foodContains } from '../../utils/foodsContains';
...
export function Home() {
  ...
  async function handleSelectImage() {
    try {
      ...
      if (status !== ImagePicker.PermissionStatus.GRANTED) {
        return Alert.alert(
          'É necessario conceder permissão para acessar seu album!'
        );
      }
      ...
      if (!response.canceled) {
        ...
        const imgManipuled = await ImageManipulador.manipulateAsync(
  ...
  async function foodDetect(imageBase64: string | undefined) {
    const response = await api.post(
      `/v2/models/${process.env.EXPO_PUBLIC_API_MODEL_ID}/versions/${process.env.EXPO_PUBLIC_API_MODEL_VENSION_ID}/outputs`,
      {
        user_app_id: {
